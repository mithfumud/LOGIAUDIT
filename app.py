...
def save_inventory(df):
    """Save inventory DataFrame to disk so it survives page reloads."""
    with open(INVENTORY_CACHE_PATH, "wb") as f:
        pickle.dump(df, f)

def load_inventory():
    """Load inventory from disk if it exists."""
...
def delete_inventory():
    """Delete cached inventory from disk."""
    if os.path.exists(INVENTORY_CACHE_PATH):
...
# ── Auto-load inventory from disk on every cold start ────────────────────
# This runs whenever session_state["inventory_master"] is None,
# i.e. on first load or after a browser refresh.
